...
| **Trust on a spoofable key** (From headers are forgeable) | Auto-execution requires the sender to be **DMARC-verified** (`items.authenticated === true`, parsed from Gmail's `Authentication-Results`). Trust earned by a sender does not transfer to an attacker who spoofs that sender — spoofed mail always falls through to manual review. |
...
| **Long-term memory poisoning** (attacker text persisted into the agent's durable memory) | The reflection loop that writes the constitution treats all evidence as **untrusted** (fenced), is instructed to encode *user-behavior patterns* (sender + action), never to copy raw item text into a rule, and never to turn an item's own words into a rule. The **golden-set eval gate** independently rejects any constitution rewrite that regresses past decisions. |
